...
private:
  ...
  handle_go_home_cancel(const std::shared_ptr<GoHomeGoalHandle> goal_handle);

  void
  handle_go_home_accepted(const std::shared_ptr<GoHomeGoalHandle> goal_handle);

  void execute_go_home(const std::shared_ptr<GoHomeGoalHandle> goal_handle);

  rclcpp_action::GoalResponse
  handle_move_cube_goal(const rclcpp_action::GoalUUID &uuid,
  ...
  void handle_move_cube_accepted(
      const std::shared_ptr<MoveCubeGoalHandle> goal_handle);
